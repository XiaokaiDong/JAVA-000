...
终于完成了网关。第10周没有必做作业，所以花大部分时间完成了网关。20201023

在预热后，性能还可以超过HTTPCLIENT4，但这个压测结果太过于简单了。在高并发下（>40个并发，每个并发1500个请求，性能会下降，链接池出现阻塞）。当允许建立120个链接时，并发<40时，链接池的连接数用不满。

搭建了一个简单的连接池。经过简单的压测，在其他条件相同的情况下，性能强于同样采用异步方式的HTTPCLIENT4。
